...
public class ListaController {
	...
	public void searchContato() {
		...
		if (contato != null) {
			...
			System.out.printf(
					"\nNome: %s - Telefone: %s\n\nComparacoes = %d\n", current
							.getData().getNome(), current.getData()
							.getTelefone(), contatos.getComparacoes());
		}
	}
	...
	public void travessia(int travessia) {
		switch (travessia) {
		case 1:// "prefix":
	...
	private void prefix(Nodo<Pessoa> localraiz) {
		if (localraiz != null) {
			view.printContato(localraiz.getData().getNome(), localraiz
					.getData().getTelefone());
	...
	public void buscaLargura() {
		if (contatos.getRaiz() == null)
			...
			while (q.peek() != null) {
				...
				view.printContato(temp.getData().getNome(), temp.getData()
						.getTelefone());
